...
@section('content')
@if(session('success'))
<div id="success-message" class="bg-green-100 text-green-700 p-3 rounded mb-4 justify-center">
    {{ session('success') }}
</div>
...
<div class="p-4 md:p-8 mt-20">
    <div class="bg-white rounded-md w-full py-6 md:py-10 px-4 md:px-10">
        ...
        <div class="overflow-x-auto">
                            ...
                            if ($dp >= 0 && $dp <= 25) { $prioritas='Rendah' ; $color='text-green-600 font-semibold' ; }
                                elseif ($dp>= 26 && $dp <= 50) { $prioritas='Sedang' ;
                                    $color='text-yellow-600 font-semibold' ; } elseif ($dp>= 51 && $dp <= 75) {
                                        ...
                                        76) {
                                        $prioritas = 'Urgent'; $color = 'text-red-600 font-semibold';
                                        } else {
                                        $prioritas = '-'; $color = '';
                                        }
                                        @endphp
            ...
            <!-- Pagination -->
            <div class="flex flex-col md:flex-row justify-end items-center mt-4 gap-2">
                <div class="join grid grid-cols-2 gap-2">
                    <button id="prevPageBtn" class="join-item btn btn-outline bg-primary text-white" disabled>
                        Previous
                    </button>
                    <button id="nextPageBtn" class="join-item btn btn-outline bg-primary text-white">
                        Next
                    </button>
                </div>
            </div>
...
<div id="detailModal" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-lg w-full max-w-lg md:max-w-2xl max-h-[80vh] overflow-y-auto p-4 md:p-6 shadow-lg">
        ...
        <div class="flex flex-wrap gap-2 justify-end">
            ...
            <button id="rejectButton" data-id="" data-id-user="" onclick="openRejectModal()"
                class="px-4 py-2 bg-red-500 text-white rounded hover:bg-red-600">
                Tolak
            </button>
            ...
            <button id="addNoteButton" data-id="" data-id-user="" onclick="openAddNoteModal()"
...
<div id="deleteModal" class="fixed inset-0 bg-white bg-opacity-50 hidden z-50 flex items-center justify-center">
    <div class="bg-white p-6 rounded-lg w-[90%] md:w-1/3 max-h-[80vh] overflow-y-auto mx-4">
...
<div id="addNoteModal" class="fixed inset-0 bg-white bg-opacity-50 hidden z-50 flex items-center justify-center">
    <div class="bg-white p-6 rounded-lg w-[90%] md:w-1/3 max-h-[80vh] overflow-y-auto mx-4">
        ...
        <form action="{{ route('update-repair-notes-admin') }}" method="POST">
            @csrf
...
<div id="scheduleModal" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden flex items-center justify-center">
    <div class="bg-white w-full max-w-md rounded-lg shadow-lg p-6 mx-4">
        <form action="{{ route('update-schedule-admin') }}" method="POST">
            @csrf
...
<div id="teknisiModal" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden flex items-center justify-center">
    <div class="bg-white w-full max-w-md rounded-lg shadow-lg p-6 mx-4">
        <form action="{{ route('create-teknisi') }}" method="POST">
            @csrf
...
<div id="rejectModal" class="fixed inset-0 bg-white bg-opacity-50 flex items-center justify-center hidden z-[9999]">
    <div class="bg-white rounded-lg shadow-lg w-full max-w-md p-6 mx-4">
        <form action="{{ route('reject-repair-admin') }}" method="POST">
            @csrf
...
<script>
    ...
    function showDetail(id) {
        ...
        rejectButton.setAttribute('data-id', laporan.id);
        ...
        if (status === 'selesai') {
            ...
            if (status === 'dalam proses pengerjaan' || status === 'pengecekan akhir') {
                ...
                if (laporan.latest_history && laporan.latest_history.id) {
                    addNoteButton.setAttribute('data-id', laporan.latest_history.id);
    ...
    `;
        ...
        document.getElementById('editForm').action = `/laporan/${laporan.id}`;
        ...
        document.getElementById('deleteForm').action = `/delete-report-admin/${laporan.id}`;
    ...
    function openAddNoteModal() {
        const id = document.getElementById("addNoteButton").getAttribute("data-id");
        const id_user = document.getElementById("addNoteButton").getAttribute("data-id-user");
        ...
        document.getElementById("historieId").value = id;
    ...
    function openRejectModal() {
        const id = document.getElementById("rejectButton").getAttribute("data-id");
        const id_user = document.getElementById("rejectButton").getAttribute("data-id-user");
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        entriesInput.addEventListener('input', () => {
            entriesPerPage = parseInt(entriesInput.value) || 10;
            currentPage = 1;
            renderTable();
        });
        ...
        searchInput.addEventListener('input', () => {
            ...
            filteredRows = rows.filter(row => {
                const pengajuan = row.children[1]?.textContent.toLowerCase() || '';
                const gedung = row.children[2]?.textContent.toLowerCase() || '';
                return pengajuan.includes(q) || gedung.includes(q);
            });
            currentPage = 1;
            renderTable();
        });
