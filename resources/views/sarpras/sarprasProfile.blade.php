...
@section('content')
<div class="px-4 py-6 md:px-8 mt-20">
    <div class="bg-white shadow-lg rounded-lg w-full max-w-4xl mx-auto p-6 md:p-8">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-primary font-bold text-2xl">Profil Pengguna</h1>
        </div>
        @if(session('success'))
        <div id="success-alert"
            class="bg-green-100 w-full md:w-1/2 text-green-800 flex justify-center p-2 mb-4 rounded mx-auto">
            {{ session('success') }}
        </div>
        ...
        <div class="space-y-4 gap-y-2">
            <div class="flex flex-col md:flex-row md:items-center">
                <span class="text-sm font-medium text-gray-700 w-full md:w-48">Nama Lengkap</span>
                <span class="text-sm text-gray-900">{{ $user->name }}</span>
            </div>

            <div class="flex flex-col md:flex-row md:items-center">
                <span class="text-sm font-medium text-gray-700 w-full md:w-48">Email</span>
                <span class="text-sm text-gray-900">{{ $user->email }}</span>
            </div>

            <div class="flex flex-col md:flex-row md:items-center">
                <span class="text-sm font-medium text-gray-700 w-full md:w-48">Role</span>
                <span class="text-sm text-gray-900">{{ $user->role }}</span>
            </div>

            <div class="flex flex-col md:flex-row md:items-center">
                <span class="text-sm font-medium text-gray-700 w-full md:w-48">Tanggal Dibuat</span>
                <span class="text-sm text-gray-900">{{ $user->created_at }}</span>
            </div>

            <div class="flex flex-col md:flex-row md:items-center">
                <span class="text-sm font-medium text-gray-700 w-full md:w-48">Terakhir Diperbarui</span>
    ...
    <div id="editProfileModal"
        ...
        <div class="bg-white p-6 rounded-lg w-full max-w-md shadow-xl mx-4">
            ...
            <form id="editProfileForm" action="{{ route('update-profile-sarpras') }}" method="post">
                @csrf
                @method('PATCH')
                <div class="mb-3">
    ...
    <div id="changePassword" class="fixed inset-0 z-50 hidden bg-black bg-opacity-50 flex justify-center items-center">
        <div class="bg-white p-6 rounded-lg w-full max-w-md shadow-xl mx-4">
            ...
            <form id="changePasswordForm" action="{{ route('change-password-sarpras') }}" method="POST">
                @csrf
                ...
                <div class="mb-3 relative">
                    ...
                    <div class="absolute right-3 top-[2.6rem] cursor-pointer"
