...
<div class="p-4 sm:p-8 max-w-4xl mx-auto">
    <div class="bg-white shadow-lg rounded-lg w-full p-6 sm:p-8">
        ...
        <div id="success-alert"
            class="bg-green-100 w-full sm:w-1/2 text-green-800 flex justify-center p-2 mb-4 rounded mx-auto text-center">
            {{ session('success') }}
        </div>
        ...
        <div class="space-y-4">
            ...
            ['label' => 'Tanggal Dibuat', 'value' => $user->created_at],
            ['label' => 'Terakhir Diperbarui', 'value' => $user->updated_at]
            ] as $item)
            <div class="flex flex-wrap sm:flex-nowrap items-center">
                <span class="text-sm font-medium text-gray-700 w-full sm:w-48 mb-1 sm:mb-0">{{ $item['label'] }}</span>
                <span class="text-sm text-gray-900 w-full sm:flex-1 break-words">{{ $item['value'] }}</span>
            </div>
            @endforeach
        </div>
        ...
        <div class="flex flex-col sm:flex-row justify-end mt-6 gap-2">
            <button id="ubahPwBtn"
                class="bg-blue-600 text-white px-6 py-2 rounded-md hover:bg-blue-700 w-full sm:w-auto">Ubah
                Password</button>
            <button id="editProfileBtn"
                class="bg-blue-600 text-white px-6 py-2 rounded-md hover:bg-blue-700 w-full sm:w-auto">Edit
                Profil</button>
        </div>
    </div>
</div>
...
<div id="editProfileModal" class="fixed inset-0 z-50 hidden bg-black bg-opacity-50 flex justify-center items-center">
    <div class="bg-white p-6 rounded-lg w-full max-w-md shadow-xl mx-4">
        <div class="flex justify-between items-center mb-4">
            ...
            <button id="closeModalEdit" class="text-gray-600 hover:text-red-600 text-2xl leading-none">&times;</button>
        </div>
        <form id="editProfileForm" action="{{ route('update-profile-user') }}" method="post">
            @csrf
            @method('PATCH')
            <div class="mb-3">
...
<div id="changePassword" class="fixed inset-0 z-50 hidden bg-black bg-opacity-50 flex justify-center items-center">
    <div class="bg-white p-6 rounded-lg w-full max-w-md shadow-xl mx-4">
        <div class="flex justify-between items-center mb-4">
            ...
            <button id="closeModalUbah" class="text-gray-600 hover:text-red-600 text-2xl leading-none">&times;</button>
        </div>
        <div id="passwordAlert" class="mb-4 hidden"></div>
        <form id="changePasswordForm" action="{{ route('change-password-user') }}" method="POST">
            @csrf
            <div class="mb-3 relative">
                <label for="old_password" class="form-label text-primary">Password Lama</label>
                <input type="password" class="w-full p-2 pr-10 border border-gray-300 rounded bg-white text-gray-700"
            ...
            fetch(form.action, {
                ...
                .then(async response => {
                    if (response.ok) {
                        ...
                        if (errorData.errors) {
                            const messages = Object.values(errorData.errors).flat().join('<br>');
